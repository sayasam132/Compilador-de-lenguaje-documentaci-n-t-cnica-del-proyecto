...
EvalResult eval(ASTNode *node) {
    ...
    /* Busca con shadowing: scope actual hacia arriba */
    case AST_IDENTIFIER: {
    ...
    /* Declara en el scope actual (shadowing automatico) */
    case AST_VAR_DECL: {
    ...
    /* Solo registra el nodo; interpreter_run ya lo registro antes */
    case AST_FUNC_DECL:
        return make_result(zero_val(), 0);
...
void interpreter_run(ASTNode *program) {
    /* Pre-pase: registra todas las funciones primero */
    ...
    /* Ejecuta sentencias no-funcion en orden */
    for (int i = 0; i < program->data.block.count; i++) {
        ASTNode *stmt = program->data.block.stmts[i];
        if (stmt && stmt->type != AST_FUNC_DECL)
            eval(stmt);
